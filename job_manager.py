...
async def mark_expired_jobs(table_name: str):
    ...
    try:
        response = await supabase.table(table_name)\
        ...
        # Proper handling of PostgREST response is needed here
        logging.info(f"Marked expired jobs in {table_name}. Response: {response}")
...
async def check_linkedin_job_activity(table_name: str):
    ...
    # ... (logic remains similar but uses table_name)
...
async def archive_old_jobs(table_name: str):
    ...
    inactive_statuses = ['expired', 'removed'] # From previous steps
    ...
    try:
        ...
        logging.info(f"Archived old jobs in {table_name}. Response: {response}")
...
async def run(table_name: str):
    ...
    # 1. Mark old jobs as expired
    await mark_expired_jobs(table_name)
    
    # 2. Check for removed/inactive jobs (LinkedIn specific)
    # We check the table name to ensure this only runs for LinkedIn.
    if table_name == config.SUPABASE_TABLE_MAP.get("linkedin"):
        await check_linkedin_job_activity(table_name)
    else:
        logging.info(f"Skipping activity check for non-LinkedIn source: {table_name}")
        
    # 3. Archive very old, inactive jobs to clean up the main view
